...
const pByte = require("../utils/prettyBytes")
const sh = require("./sh")
const urlFileDownload = require('./urlFileDownload');
...
function bot(torrent, bot) {
  ...
  bot.onText(/\/mirror (http.+)/, (msg, match) => {

  const chatId = msg.chat.id;
  const resp = match[1];
  
  try{
    const intr = resp.split("/");
    const name = intr[intr.length - 1];
    
     urlFileDownload(resp , './downloads',false, (e)=>{
       console.log(e)
...
bot.onText(/\/mirror (http.+) (.+)/, (msg, match) => {
  ...
  try{
    ...
    const name = match[2];
     ...
     urlFileDownload(resp , './downloads',name, (e)=>{
       console.log(e)
    bot.sendMessage(chatId, e)
  },
  ()=>{})
